...
void L1TStage2EMTF::bookHistograms(DQMStore::IBooker& ibooker, const edm::Run&, const edm::EventSetup&) {
  ...
  cscDQMOccupancy->setAxisTitle("10 degree Chamber (N=neighbor)", 1);
  int count=0;
  ...
  cscTimingTot->setAxisTitle("10 degree Chambers (N=neighbor)", 1);
  ...
  for (int hist = 0;  hist < 5; ++hist) {
    ...
    cscLCTTiming[hist]->setAxisTitle("10 degree Chambers (N=neighbor)", 1);
    ...
    cscLCTTimingFrac[hist]->setAxisTitle("10 degree Chambers (N=neighbor)", 1);
    for (int xbin=1; xbin < 43; ++xbin){
    ...
    rpcHitTimingFrac[hist]->getTH2F()->GetXaxis()->SetCanExtend(false);//Needed to stop multi-thread summing
